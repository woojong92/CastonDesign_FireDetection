...
char device_id[] = 		"b1272311923e46e78fd97824f61f2ed9";
char device_token[] = 		"66604db4ba1c403592ac4fd5b13f175c";
...
void onMessage(void *client, mqtt_msg_t *msg) {
    ...
    if (strncmp(strActName, "\"setOn\"", 7) == 0) {
    	printf("Turn on Sprinkler\n");
    	gpio_write(52, 0);

    	mqtt_msg_t message;
    	message.payload = (char*)"{\"state\":1}";
    	...
    	printf("Turn off Splinker\n");
    	gpio_write(52, 1);

    	mqtt_msg_t message;
    	message.payload = (char*)"{\"state\": 0}";
