...
int main(int argc, FAR char *argv[])
{
	...
	while(1){
	   ...
	   before_Temp=current_Temp;
		//current_Buf의 값을 before_Buf로 복사
	   ...
	   for(i=0; i<64; i++){
		...
		/************************이상징후 감지********************************/
			...
			//이상징후 감지 후로부터 온도상승률 판단

				//이상징후 7이상-> fire_state: Level-3
		if(cnt>6){
			...
			gpio_write(51, 1);
			//화재발생시 부저 및 LED setOn
			gpio_write(52, 0);
		}
		//이상징후 4이상 7미만-> fire_state: Level-2
		else if(cnt>3){
